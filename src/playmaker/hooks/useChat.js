...
export const useChatMessages = (chatId) => {
  ...
  const sendMessage = useCallback(async (content, currentChatId = chatId) => {
    ...
    try {
      ...
      // Add AI response with all data from backend
      const aiMessage = {
        id: response.id,
        role: 'assistant',
        content: response.content,
        chat_answer: response.chat_answer,
        structured_content: response.structured_content, // Visual cards data
        sources: response.sources, // Data sources array
        data_info: response.data_info, // Fetch metadata

...
export const parseMessageContent = (message) => {
  ...
  if (!message || !message.context) {
    return {
      text: cleanPerplexityText(message.content || ''),
      contentItems: [],
  ...
  return {
    text: cleanPerplexityText(message.content || ''),
    contentItems,
    hasStructuredContent: contentItems.length > 0
  };
};
...
export const parseChatAnswer = (chatAnswer) => {
  ...
  return {
    text: cleanMarkdownArtifacts(cleanPerplexityText(chatAnswer.text || '')),
    contentItems: contentItems.filter(item => item.type !== 'text' || item.content.trim()), // Filter empty text cards
    hasStructuredContent: contentItems.some(item => item.type !== 'text')
  };
};

...
const PerplexityStyleMessage = ({ message, isUser, isTyping = false }) => {
        ...
        <div className="flex space-x-1 border-b border-[#1a1a1a] mb-8 overflow-x-auto pb-1">
          {["All", "Videos", "Images", "Stats", "Players"].map(tab => (
            <button
              ...
              className={`pb-2 px-4 text-xs font-medium tracking-wider uppercase whitespace-nowrap transition-colors ${
                activeMediaTab === tab 
  ...
  return (
    <div className="mb-16">
      ...
      {/* 2) AI Narrative for game queries (Pulse AI style) */}
      {activeMediaTab === 'All' && text && hasStructuredContent && (
        <div className="mb-8">
          <div className="p-8 bg-[#111111] border border-[#1a1a1a]">
            <div className="flex items-center space-x-3 text-xs uppercase tracking-wider text-gray-600 mb-4">
      ...
      {/* 3) Summary paragraph - only on All tab (for non-game queries) */}
      {activeMediaTab === 'All' && text && !hasStructuredContent && (
        <div className="mb-6">
          {hasHighlights && (
            <div className="flex space-x-3 items-center text-gray-600 text-xs uppercase tracking-wider mb-6">
              ...
              {(() => { try { console.log('[AUDIT][AI_LABEL]', message?.role || 'assistant', 'label=AI (summary)'); } catch {} return null })()}
              {/* Intentionally omitting the "Answer" label */}
              {message.source && (
                <span className="text-gray-600">• {message.source}</span>
              )}
            </div>
          ...
          <div className="prose prose-invert max-w-none text-white text-base leading-relaxed space-y-4 font-light">
            <ReactMarkdown>{formatChatText(text)}</ReactMarkdown>
          </div>
        </div>
      )}
        ...
        <div className="border border-[#1a1a1a] overflow-hidden">
          ...
          <div className="flex bg-[#111111] border-b border-[#1a1a1a]">
            <button
              ...
              className={`px-5 py-3 text-xs font-medium tracking-wider uppercase transition-colors ${
                activeTab === 'response' 
...
const SourcesTab = ({ message, contentItems }) => {
  ...
  if (context.sportradar_data && !context.sportradar_data.error) {
    sources.push({
      title: 'Sportradar Sports API',
      description: 'Live sports data and statistics',
      url: 'https://sportradar.com',
      domain: 'sportradar.com'
    });
  }
  
  if (context.highlightly_data && !context.highlightly_data.error) {
    sources.push({
      title: 'Highlightly Football API',
      description: 'Football matches, highlights, and player data',
      url: 'https://highlightly.net',
      domain: 'highlightly.net'
    });
  }

  // Add Perplexity/AI source
  sources.push({
    title: 'Perplexity AI',
    description: 'AI-powered sports analysis and insights',
    url: '#',
    domain: 'AI Analysis'
  });
